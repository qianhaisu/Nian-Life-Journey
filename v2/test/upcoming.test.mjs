...
import { feedFromResult, normalizeUpcoming, sortUpcoming, UPCOMING_VISIBLE } from "../lib/upcoming.ts";
...
test("three of the feed's four no-row states render nothing, and so does the fourth", () => {
  ...
  // no_items IS a real statement about the window — and this page still declines to make it,
  // because a section that appears only to say 「没有待办」 reads as 「已经接入了，本周没事」.
  const none = feedFromResult({ state: "no_items", coverage });
  assert.equal(none.status, "unavailable");
  assert.match(none.reason, /没有待办/);
