...
export default async function YearPage({ params }: { params: Promise<{ year: string }> }) {
  ...
  return <div className="year-page reading-wrap">
    <header className="chapter-masthead">
      ...
      <p className="chapter-meta">{view.memoryCount} 段记忆 · {view.traceDayCount} 天留下了生活痕迹</p>
    </header>
    {view.months.map((month) => <section className="year-month" key={month.chapter.month} aria-labelledby={`month-${month.chapter.month}`}>
      ...
      {month.traceDayCount > 0 ? <p className="chapter-meta">{month.chapter.memories.length > 0 ? "还有" : ""} {month.traceDayCount} 天留下了生活痕迹</p> : null}
    </section>)}
