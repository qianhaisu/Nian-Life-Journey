...
export default function HomePage() {
  ...
  return <div className="home-page">
    ...
    <section className="home-changes reading-wrap" aria-labelledby="changes-title"><div className="section-heading"><span className="section-mark">最近的变化</span><h2 id="changes-title" className="serif">这几天，悄悄变了什么</h2></div><ol>{recentChanges.map((change) => <li key={change.title}><time>{change.date}</time><div><span>{change.label}</span><h3 className="serif">{change.title}</h3><p>{change.note}</p></div></li>)}</ol><Link className="text-link" href="/about">从成长的角度继续看 <span>↗</span></Link></section>
    ...
    <section className="month-encounter wide-wrap"><div><span className="section-mark">当前月份</span><p className="serif">“{monthlySnapshot.summary}”</p><Link className="text-link" href="/memory/2026/08">翻回八月 <span>↗</span></Link></div><aside><span className="section-mark">以前的这个时候</span><h2 className="serif">一年前，他还不会走。</h2><p>这是一段克制的 Mock：不是每天提醒，只在有值得重遇的内容时出现。</p><Link href="/memory">去以前看看</Link></aside></section>
  </div>;
}
