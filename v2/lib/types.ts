...
export interface RawSource { id: string; profileId: string; sourceType: SourceType; contentTypes: ContentType[]; contributorId: string; capturedAt: string; importedAt: string; text?: string; mediaIds: string[]; sourceLabel: string; visibility: Visibility; status: RawSourceStatus; originalFilename?: string; provider?: string; providerExternalId?: string; metadata?: Record<string, unknown>; /** Only on id-scoped reads (getSourcesByIds): metadata.senderDigest, projected on its own. */ senderDigest?: string | null; deletedAt?: string; relatedLifeEventId?: string; extractedMedicalFacts?: { hospital?: string; examinationType?: string; recordedAt?: string; facts: string[] }; }
...
// `precision` says how certain the figure is: "stated" when the source gave a number, "approximate"
// when it hedged. Undefined means nobody has judged it — never assume "stated".
export type GrowthPrecision = "stated" | "approximate";
export interface GrowthRecord { id: string; profileId: string; lifeEventId?: string; kind: GrowthKind; observedAt: string; value?: number; unit?: string; precision?: GrowthPrecision; note: string; source: string; visibility: Visibility; }
...
export type ChatImportTaskStatus = "pending" | "running" | "retry_pending" | "completed" | "completed_with_warnings" | "failed" | "cancelled";
export type ChatImportStage = "snapshot_validation" | "bundle_parse" | "raw_source_persist" | "media_validate" | "media_upload" | "media_link" | "finalize";
export type ChatImportWarning = { code: string; count: number };
export type ChatImportCheckpoint = { snapshotDigest: string; documentOrdinal: number; messageOrdinal: number; mediaDigest?: string };
export interface ChatImportTask { id: string; profileId: string; importBatchId: string; status: ChatImportTaskStatus; phase: ChatImportStage; currentStage: ChatImportStage; processedMessages: number; createdMessages: number; reusedMessages: number; warnings: number; warningCounts: ChatImportWarning[]; checkpoint?: ChatImportCheckpoint; leaseOwner?: string; leaseExpiresAt?: string; attempt: number; maxAttempts: number; cancelRequestedAt?: string; startedAt?: string; completedAt?: string; safeErrorCode?: string; createdAt: string; updatedAt: string; }
