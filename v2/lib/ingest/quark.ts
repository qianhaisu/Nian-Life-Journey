...
export async function ingestQuarkFile(file: QuarkFile, options: QuarkImportOptions, client?: Pick<QuarkClient, "download">) {
  ...
  const media: Media = { id: mediaId, profileId: options.profileId, rawSourceId: sourceId, mediaAssetId: assetId, type, src: mediaDeliveryUrl(mediaId, variant), originalFilename: file.filename, mimeType: file.mimeType, fileSize: file.size ?? bytes?.byteLength, alt: file.filename, takenAt: wallClockOf(file.takenAt ?? now) ?? now, visibility, width, height, durationSeconds: file.durationSeconds };
