...
import { canonicalMessageId, type ChatMessage, type ChatMessageType } from "./chat-import-bundle";
...
export function parseWechatMarkdown(input: { root: string; document: string; text: string; media: Map<string, { checksum: string; availability: "present" | "needs_review" | "invalid" }> }): ParsedMarkdown {
  ...
  const flush = () => { if (!current) return; const body = current.body.join("\n"); const refs = [...body.matchAll(/!\[[^\]]*\]\(([^)]+)\)/g)].map((m, i) => { const path = safeTarget(input.root, input.document, m[1]); const meta = path ? input.media.get(path) : undefined; return { id: `media-ref:${id(path ?? m[1])}:${i}`, relativePath: path ?? m[1], checksum: meta?.checksum, availability: meta?.availability ?? "missing" as const }; }); const type: ChatMessageType = refs.length ? (body.replace(/!\[[^\]]*\]\(([^)]+)\)/g, "").trim() ? "mixed" : "image") : "text"; const base = { conversationId, senderId: `sender:${id(current.sender)}`, direction: "unknown" as const, sentAt: current.sentAt, messageType: type, text: body, mediaRefs: refs, sourceLocator: { document: input.document, recordOrdinal: current.ordinal } }; messages.push({ ...base, messageId: canonicalMessageId(base, 0) }); };
  for (const line of lines.slice(boundary + 1)) { const match = line.match(header); if (match) { flush(); ordinal += 1; current = { sentAt: `${match[1]}-${match[2]}-${match[3]}T${match[4]}:${match[5]}:${match[6]}+08:00`, sender: match[7], body: [], ordinal }; } else if (current) current.body.push(line); }
