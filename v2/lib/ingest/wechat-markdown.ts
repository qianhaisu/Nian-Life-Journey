...
// `sessionKey` is the chat's own id as the exporter wrote it (Markdown: 会话ID, JSON: session.wxid),
// NOT the document-derived conversationId. One chat exported twice has two conversationIds and one
// sessionKey, which is the only handle that can say "these two documents are the same chat".
export interface ParsedMarkdown { document: string; conversationId: string; conversationName: string; sessionKey: string; messages: ChatMessage[]; warnings: string[]; }
...
export function parseWechatMarkdown(input: { root: string; document: string; text: string; media: Map<string, { checksum?: string; availability: MediaAvailability; mimeType?: string; fileSize?: number; width?: number; height?: number }> }): ParsedMarkdown {
  ...
  const title = lines[0]?.match(/^# (.+)$/)?.[1]; if (!title) return { document: input.document, conversationId: "", conversationName: "", sessionKey: "", messages: [], warnings: ["auxiliary_markdown"] };
  const boundary = lines.indexOf("---"); if (boundary < 0) return { document: input.document, conversationId: id(title), conversationName: title, sessionKey: "", messages: [], warnings: ["missing_separator"] };
  ...
  return { document: input.document, conversationId, conversationName: "conversation_1", sessionKey: sessionId ? `wechat-session:${sessionId}` : "", messages, warnings: [] };
}
