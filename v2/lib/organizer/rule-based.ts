...
export async function organizeSources(sourceIds: string[]): Promise<OrganizerResult> {
  ...
  const eventId = existing?.id ?? newId("event"); const scopes = (medical ? ["family"] : ["family", ...(sources.some((source) => source.sourceType.startsWith("daycare")) ? ["daycare"] : [])]) as LifeEvent["scopes"]; const event: LifeEvent = existing ? { ...existing, contentTypes: [...new Set([...existing.contentTypes, ...contentTypes])], mediaIds: [...new Set([...existing.mediaIds, ...mediaIds])], sourceIds: [...new Set([...existing.sourceIds, ...sourceIds])], heroMediaId: existing.heroMediaId ?? mediaIds[0] } : { id: eventId, profileId: store.profile.id, title, story, occurredAt: date, people: contributorNames as string[], tags: contentTypes, contentTypes, mediaIds, sourceIds: [], growthRecordIds: [], careRecordIds: [], eventType: medical ? "routine" : sources.some((source) => source.sourceType.startsWith("daycare")) ? "routine" : "moment", memoryWeight: medical ? "trace" : mediaIds.length > 5 || hasVideo ? "memory" : "trace", scopes, heroMediaId: mediaIds[0], visibility: medical ? "private" : "family", keptInYearbook: false, createdBy: "rule", organizerVersion: "rule-v1" };
