...
export const AI_ORGANIZER_EVALUATION_FIXTURES: EvaluationFixture[] = [
  ...
  // Id kept as "attach-video" so historical reports still resolve, but this fixture has no text and
  // no usable media evidence (mediaCount 0) — only a same-day date and a partial content-type overlap
  // with an existing memory, which isn't enough to safely target one. It verifies that a video with
  // no text/media evidence is stored conservatively, not that attach_existing linking works; see the
  // dedicated attach_existing coverage in test/ai-organizer.test.mjs for that.
  { id: "attach-video", description: "A video with no text or usable media evidence is stored conservatively, not auto-attached", expectedActions: ["daily_trace", "store_only"], context: context({ sourceSummaries: [{ ...photo("eval-video", 0, ["motor", "family"]), sourceType: "family_video" }], existingMemories: [{ id: "eval-existing-ball", occurredAt: "2026-08-28", title: "托班追球", story: "老师记录了追球活动。", contentTypes: ["daycare", "motor"], memoryWeight: "memory", sourceCount: 2, visibility: "family" }] }) },
