...
// Every response is uncacheable. There is no health login: reads are open like the rest of the site; a write needs a same-origin Origin header and
// a declared entry person (X-Health-Entry-By: mom|dad - recorded as declared, NOT a verified identity).
import { entryBy, sameOrigin } from "./auth";
...
const HEADERS = { "Cache-Control": "no-store, private", "X-Content-Type-Options": "nosniff", Vary: "Origin", "Referrer-Policy": "no-referrer" };
...
export function createHealthRecordHandler(getConfig: () => ConfigResult = () => loadHealthRecordConfig(), serviceOpts: () => ServiceOptions = () => ({}), pageSources: (cfg: HealthRecordConfig) => PageSourcesConfig = (cfg) => loadPageSourcesConfig(cfg.repo)) {
  ...
  return async function handle(req: Request, segments: string[]): Promise<Response> {
    ...
    try {
      ...
      const by = entryBy(req);
      if (write && !by) return json(400, { ok: false, code: "entry_by_required", message: "请先选择录入人（妈妈或爸爸）。" });
      ...
      if (a === "entries" && !b) {
        ...
        if (method === "POST") return json(200, { ok: true, ...(await create(service, by!, req)) });
      }
      ...
      if (a === "entries" && b && c === "corrections" && method === "POST") return json(200, { ok: true, ...(await service.correct(by!, decodeURIComponent(b), (await readJson(req)) as Record<string, unknown>)) });
      if (a === "entries" && b && c === "attribution" && method === "POST") return json(200, { ok: true, ...(await service.setAttribution(by!, decodeURIComponent(b), (await readJson(req)) as Record<string, unknown>)) });
