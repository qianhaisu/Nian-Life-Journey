...
import { captureSources } from "@/app/actions";

const kinds = [
  { value: "family_photo", label: "照片 / 视频" },
  { value: "wechat", label: "聊天记录" },
  { value: "daycare", label: "托班记录" },
  { value: "medical_document", label: "文档 / 就医资料" },
...
export function MemoryInbox() {
  ...
  return <div className="capture-flow" data-ai-id="capture-organize-flow">
    <div className="capture-kinds" role="list" aria-label="资料类型">
      {kinds.map((item) => <button type="button" role="listitem" className={kind === item.value ? "is-selected" : ""} key={item.value} onClick={() => setKind(item.value)} aria-pressed={kind === item.value}><span aria-hidden="true">{kind === item.value ? "✓" : "＋"}</span>{item.label}</button>)}
    </div>
    {!result ? <form className="capture-form" action={(formData) => {
      setMessage("");
