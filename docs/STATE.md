...
## 用户最新决定（2026-09-11，覆盖下方摘要中与之冲突的部分；历史全部保留）

三条，直接影响现在做什么：

1. **ICP 备案尚未通过。** 不要把「已通过」当前提，不改 DNS，不开公网入口，不做公开切换。
   下方第 4 步「准备公开切换」的条件仍然不满足，但准备工作可以继续做。
2. **周末继续私有内容建设。** 交付面是 loopback + SSH 隧道上的私有站，以及本机可读的试读包，
   不是 nianlife.cn。「完成」的判断标准仍然是家人能不能读，只是读的地方暂时是私有站。
3. **2025 与 2026 两年都要推进**，不是只做 2025。2026 优先 8 月与 9 月的已有材料，
   再挑一个较早月份做连续阅读样本；2025 把已有档案和上一轮复核通过的 64 篇合并起来读。

配套边界（同一次指示）：审核状态一律保持现状，不自动 approved、不发布；未复核的新内容单独标
「待复核」，不混进已通过的试读集合；不覆盖正式 `monthly_snapshot` 来偷渡初稿；Organizer 后台
开关保持关闭，有界工作用显式名单执行；家庭正文与照片放 NianlifeOps 目录，不进 Git。

**2026-09-11 深夜这一轮按上面三条执行的结果**见 [`docs/STATUS.md`](STATUS.md) 末节：
两年私有试读入口（`/preview/2025`、`/preview/2026`，以及本机 `round7-2026-09-11/read2/`）、
2026-09-04 → 09-09 五篇新草稿、七篇修订、两份月度回顾初稿、11 行「这张拍的是他」。
已知最大 blocker 也在那一节：**管线仍不是无人值守**，最近一周的内容是人手写的。
