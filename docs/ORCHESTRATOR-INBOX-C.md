...
状态：queued | 关联：CMD-20260907-001 | 发送：Cowork | 接收：Code C | 类型：任务卡

**目标**：仅离线核对迁移依赖、人工发布/回滚检查点、worker 默认停用及成本边界，列出已有/缺失证据。

**允许修改路径**：新建 `docs/migration-C-readiness.md`（唯一产出）。

**禁止操作**：
- 不采购 RDS/OSS/ECS 等任何云资源
- 不部署、不改 DNS、不连生产数据库
- 不执行任何 Vercel 部署操作
- 不修改 worker 配置（只记录当前状态）

**前置依赖**：读 `docs/COORDINATION.md` §"迁移顺序与当前基线"节

**交付**：`docs/migration-C-readiness.md`，包含：
- 迁移各阶段依赖清单（已满足 / 未满足 / 证据缺失）
- 人工发布检查点（生产部署批准流程、DNS 切换步骤、回滚点）
- worker 默认停用状态核查（`nianlife-worker.mjs` 当前是否有定时任务在跑）
- 成本边界（已知 Neon/Vercel 成本上限，缺失的采购决策）
- Phase 2 生产备份完成的前置条件（Neon 恢复访问后 next step）

**验收方法**：Cowork 读 `docs/migration-C-readiness.md`，确认每条依赖有证据或明确标注"证据缺失"。

**资源占用**：只读代码库和文档，无网络需求。

**收到此任务请在 `docs/STATUS-C.md` 写 ACK 一行，格式：`MIG-C-001 ACK YYYY-MM-DD HH:MM CST`**
...
- Teddy 已经把 Neon 从 Launch **降级到 Free**。本计费周期（9/5–10/1）的出站流量早就用掉
  1.37 TB，而 Free 版每月只有 5 GB，**已经超额 270 多倍**，所以数据库现在是硬性拒绝连接的：
  任何连接都会直接收到 `Your project has exceeded the data transfer quota.`（Cowork 已实测确认）。
- **这是 Teddy 主动做的决定，不是故障，不是 bug，不要去"修"它。** 起因是一天之内烧掉 $87.86，
  他明确说过在彻底修好之前不会再付一分钱。
- 数据没有丢。Neon 的限额只会封操作、不会删数据。
- 网站因此打不开，这是预期内的。缓存过期后页面会陆续变成报错。

**所以，从现在起，所有轨道（A / B / C）一律停工待命，具体禁止**：

1. **不要跑任何连生产库的东西**——查询、巡检脚本、导入、worker、`nianlife-status.mjs`，全部停。
   跑了也只会拿到上面那条配额报错，除了制造噪音没有任何用。
2. **不要访问 nianlife.cn 去"验证"**。站是死的，验证不出任何东西。
3. **不要因为看到数据库报错就去改代码、改连接串、改 `.env.local`、或者"顺手优化"什么**。
   报错的原因在账单，不在代码。
4. **不要 push**。push 会触发 Vercel 构建，构建时取数会失败，只会白白堆一串失败部署。
   本地 commit 可以，push 等 Cowork 明确说可以再说。
5. 手上如果有没写完的报告，写完就停，**不要开任何新任务**。

**P0 事故本身已经收尾**（不需要任何人再去动它）：A 轨的查询修复 `7d7fe15` 已在 `main` 上；
C 轨的 Ignored Build Step 修复已生效；C 轨的 stopgap 已干净撤回（`5684d91`）。Cowork 已经独立
从 GitHub 重新 clone 复核过 `main` 上的代码状态：`loadFamilyArchive()` 确实已经改用
`getAllEventIdentities()`，5 个页面的 `revalidate` 也都干净回到 300，没有误伤。

**什么时候复工**：等 Teddy 决定恢复 Neon（10 月 1 号计费周期自动重置，或者他主动升级）。
这个决定权只在 Teddy 手上，任何 session 都不要替他做、也不要催他。Cowork 会在恢复后发新指令。
