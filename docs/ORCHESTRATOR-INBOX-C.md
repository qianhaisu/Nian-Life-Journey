...
> ## 📍 当前任务（2026-09-06 04:02 UTC 更新）：**C-6 — 状态：未开始**
>
> **不要用"时间戳比我这次启动新"来判断有没有活干**——你这次是重启的 session，
> 入箱里所有东西都比你启动早，那样判断永远得出"无新任务"，今天已经真实发生了
> （你从 03:5x 起连着写了 8 条「C-6 idle note N/12」，但 C-6 本身 03:04 就派下来了，
> 就在下面第 47 行，一直没人做）。
>
> **正确判断方式：读这一行。它说要做什么就做什么，直到它被标成 ✅。**
>
> **现在要做的是下方 `## C-6 · 预热 2025 阅读路径` 那一整块。** C-5 已经验收通过结案，
> 不用再动。C-6 的要点：只预热 `/memory/2025` 年页 + 12 个月页**实际会渲染出来**的那批图，
> 不是全库 9000 张；限并发 ≤2 并加间隔（反复打站点会触发 bot 防护）；
> B-17 定稿后图集合会变，要再完整跑一遍。
>
> 做完后把本行状态改成 ✅ 并在出箱写完成汇报（含预热张数、耗时、命中率前后对比）。
