...
>
> 最后更新：2026-09-06 04:1x · by Claude Code（A-9 完成 predeclare：13 条重写草稿 + 防复发方案，等确认）
...
**A-9（13 条重写 predeclare + 防复发方案）已完成，等确认（2026-09-06）**：
13 条（A-7 的 7 + A-8 的 6）全部给出「原始证据 / 现在的 title+story / 重写后的 title+story」
三段对照，写进 `docs/STATUS.md` 04:18 UTC 条目。分三组：A 组 10 条重写后降级痕迹层；
B 组 2 条（「张小年基本吃完了炒面」「一个人晃出了镜头」）我建议直接撤销发布而不是降级——
这两条连"核心事实有没有发生"证据都不支持（一条证据只是问句没有答案，一条证据主语根本
不是张年），跟 A 组"事实真、细节假"不是一回事；C 组 1 条（张冠李戴那条）撤销发布不给
痕迹层标记。另给了 3 个防复发方案（不动代码），倾向"发布前逐字校验引号/数字是否见于
证据"这条。**没有写库**，等 Cowork 逐条确认。
