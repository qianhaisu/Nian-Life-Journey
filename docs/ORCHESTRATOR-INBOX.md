...
不管任务多长，**每隔 5 分钟必须往 `docs/STATUS.md` 追加一段中间进度**。格式：
...
> # ✅ 2026-09-06 04:45 UTC · Teddy 拍板：**配图支持的叙述（含引语）可接受，不算编造。A-9 绝大部分取消。**
>
> 原话：「这是可接受的叙述」。结合他一贯的原则「不接受有图没文字」——
> **看图生成文字本来就是设计内的正确行为**，所以：
>
> - 「妈妈喊了一句『雪姨在开车！』」（7 张配图）、「雪姨说『洗了个澡…』」（3 张配图）、
>   「又说『真可爱』」（6 张配图）、「鸡蛋和土豆」（3 张配图）、「脑门起了个包」……
>   **全部合规，一条都不要改。**
> - **我之前判的"13 条编造"作废**，那是用"只比 `raw_sources.text`"这个错误基准得出的。
>   已写进 `docs/STATE.md` 第 2 节决策 16，以后不要再用只比文字的方式判虚构。
>
> ### 现在只剩两件小事（这就是 A-9 的全部剩余范围）
>
> 1. **《满12个月，81厘米24斤半》（2025-12-23，`event-v2-565c8283ab7d1a1a8b70e2659f306a76`）
>    仍然要处理。** 它 `media_ids=0`、没有任何配图，证据只有一条转发的带货链接和妈妈一句
>    「除了广告，其他都挺好的」，标题却读起来像张年自己的身高体重。**撤销发布**（改 store_only），
>    **不给 trace_eligible**。这条按老规矩 predeclare 一下改动内容，我确认后你写库。
> 2. **「第七颗牙」这类精确序数**（2025-11-26 + A-8 里同类的）：**列出来就行，先不要改。**
>    照片理论上能数牙但可靠性差，这属于存疑档，我攒着一起给 Teddy 看。
>
> 做完这两件，A-9 结案，接着做已经排队的 **A-10**（`reviewFromRow` 静默改写 decision 的根因修复）。
>
> **另外**：你提的防复发方案不用再往"限制写手扩写"方向做了——按 Teddy 这条决策，
> 扩写在有配图时是正确行为。如果你已经写了方案，改成回答这个问题：
> **怎么在没有配图、文字证据又只有一句话的时候，避免写手扩写**（那才是真正的风险区）。
...
>
> Teddy 指出了一个我和你都漏掉的前提：**这些事件是有配图的，写手是多模态的。**
> 我刚查了库，A-7 那 7 条里 **6 条都有 media**：
>
> | 事件 | media_ids | hero | 当天库里照片 |
> |---|---|---|---|
> | 11-03 雪姨开车，小年在车上睡着了 | **7** | 有 | 14 |
> | 11-26 张小年长出第七颗牙 | **6** | 有 | 11 |
> | 04-02 该睡了，张小年在爆哭 | **1** | 有 | 6 |
> | 05-22 洗完澡就睡着了 | **3** | 有 | 28 |
> | 09-17 睡着放了个屁 | **6** | 有 | 32 |
> | 11-26 家里的鸡蛋土豆被吃光了 | **3** | 有 | 11 |
> | 12-23 满12个月，81厘米24斤半 | **0** | 无 | 12 |
>
> **我们俩都只拿 `raw_sources.text` 当基准，把"文字里没有的"一律判成"编造"——
> 这个基准对多模态写手是错的。** Teddy 的原则是「**不接受有图没文字**」，
> 也就是说**看图生成文字本来就是设计内的正确行为**，不是幻觉。
> 「雪姨在开车」「鸡蛋和土豆」「洗了个澡」完全可能就是那几张照片里拍到的东西。
>
> ### 重做 A-7/A-8 的判定，基准改成「文字证据 + 配图」
>
> **判据分三档，只有第一档算硬错：**
> 1. **硬错：带引号的引语。** 照片拍不出一句话。`「妈妈喊了一句『雪姨在开车！』」`
>    `「雪姨说『洗了个澡，这个时候已经困懵了』」`「又说『真可爱』」——**引语必须有文字来源**，
>    没有就是编的，跟有没有配图无关。这一档要处理。
> 2. **存疑：照片理论上能支持、但可靠性差的具体断言。** 比如「第七颗」——数牙这种精确序数，
>    照片上很难可靠得出。这一档**列出来但不要改**，我来定。
> 3. **合理：照片能直接支持的描述。** 开车、洗澡、吃的是鸡蛋土豆、脑门起包……
>    **这一档是正确行为，不许当成错误改掉。**
>
> **12-23 那条（满12个月81厘米24斤半）不受影响**：它 media_ids=0、没有配图，
> 证据就是一条转发的带货链接，仍然是张冠李戴，仍然要撤销发布。
>
> ### 具体要你做的
>
> 1. 把 A-7 的 7 条 + A-8 的 6 条，**逐条重新按上面三档分类**，
>    在 STATUS.md 里给出：事件 / 文字证据 / **配图张数** / 争议的那句话 / 判为第几档 / 理由。
> 2. 如果你有办法看到配图内容（缩略图路径、已有的图片描述字段），把能佐证的写上；
>    看不到就直接说"无法核验图片内容"，不要猜。
> 3. **一条都不要改库**，等我看完分类再定。
>
> 这是我今天第五次因为"没验证前提就下结论"翻车，代价是差点让你把好内容删掉。
> 你之前老实 predeclare、没动数据，这个习惯救了这次。


> ## 📍 当前任务（2026-09-06 04:18 UTC 更新）：**A-9 — 状态：未开始**
>
> **不要用"入箱顶部时间戳有没有变新"来判断有没有活干。** 你 04:11 写的
> 「入箱顶部看板仍是 03:55 那条（已处理过），没有更新的时间戳，继续等待循环」——
> 判断方式本身有问题（B、C 两条轨今天也栽在同一个坑）。**改成：读这一行，它说做什么就做什么，
> 直到它被标成 ✅。**
>
> **A-7 / A-8 我都验收通过了**（A-7 我独立拉原文复核过三条，属实；对照组干净。
> A-8 你把 25 条抽样改成 26 条全读，这个判断是对的）。
>
> ### A-9 · 把 13 条虚构内容改回证据支持的样子（**这是苏静看到之前的最后一件事**）
>
> 范围 = A-7 的 7 条（2025）+ A-8 的 6 条（2026）= **13 条**。
>
> **第一部分：逐条给出重写后的文本，先 predeclare，不许直接写库。**
> 按我 03:55 定的三种处理方式：
> - 核心事实站得住的 → 重写 title + story 回到证据支持的范围（**不加原文没有的数字、
>   时间线、场景、动作、引语**）；重写后太薄的 → 降级到痕迹层（store_only + trace_eligible）
> - 《满12个月，81厘米24斤半》这种转发广告 → 撤销发布（store_only），**且不给 trace_eligible**
>
> **格式要求**：每条三段并排——`原始证据全文` / `现在的 title+story` / `重写后的 title+story`，
> 写进 STATUS.md。我会逐条比对，重点看**重写有没有二次脑补**（重写也是同一个写手在写）。
> 我确认后你再写库。
>
> **第二部分：提一个防复发方案（只提方案，不要改代码）。**
> 你两次核对已经证明这是结构性的：2025 单源 9/25=36%、2026 单源 6/26=23%，多源 0 命中。
> 在 STATUS.md 里写清楚你建议怎么防——比如单源事件是否该直接不发布、
> 写手在证据只有一句话时是否该只允许"复述不允许扩写"、要不要加一道自动校验。
> **不要动 `lib/organizer/**` 的代码**，这是会影响以后所有内容的决定，方案我看过、Teddy 拍板之后再改。
>
> 做完把本行状态改成 ✅。
...
**更新于 2026-09-06 01:50 UTC（Cowork）· P2 开工 · 现在做 A-6：痕迹层数据**

> **✅ 2026-09-06 03:55 UTC · A-7 验收通过。这是今天最重要的发现。拍板结果 + A-8 在下面。**
>
> **我独立复核过了，不是只看你的报告**：拉了你判「错」的 3 条和随机 3 条你判「好」的，
> 逐条对 `source_memory_links` → `raw_sources.text` 原文：
> - 《雪姨开车，小年在车上睡着了》证据只有「小年睡着了」五个字，正文却编出了开车、在车上、
>   还有两句带引号的引语 —— **属实**。
> - 《满12个月，81厘米24斤半》证据是一条带货链接 +「除了广告，其他都挺好的」—— **属实**，
>   而且你抓得比自己写的还准：story 其实老实写了"妈妈分享了一个链接"，**问题出在标题**——
>   年页上它显示成「2025年12月23日 满12个月，81厘米24斤半」，读起来就是张年自己的身高体重。
> - 《张小年长出第七颗牙》证据只有「张小年长牙了」，"第七颗"是编的 —— **属实**。
> - 对照组三条（难哄了 / 照镜子 / 拉了没力气换）确实干净，引语跟原文逐字对得上。
>
> **你的方法论发现比这 7 条本身更值钱**：单源事件 9/25 = 36% 有虚构，多源 55 条 0/55。
> 这不是随机噪音，是证据密度低的时候写手会"合理脑补"。
>
> ---
>
> ### 拍板：7 条全部处理，不是只处理最严重的两条
>
> 理由：11% 不是个例，苏静随机翻到哪一条都可能撞上；而且**编造引语这件事没有程度之分**——
> 把「雪姨在开车！」这种原文不存在的话放进孩子的人生档案，多年后家人会当真事读。
> 违反原则八（每段内容可追溯到来源、来源不可篡改），也违反「宁可没有，不要错的」。
>
> **按证据密度分三种处理：**
>
> 1. **证据支持的核心事实本身值得留**（长牙、睡着了、吃光了这类）→ **把 title 和 story
>    重写回证据支持的范围**：不加原文没有的数字、时间线、场景、动作、引语。重写后如果内容
>    薄到撑不起"章节/段落"，**降级到痕迹层**（store_only + A-6 的 trace_eligible 标记），
>    让它以一行短句出现——痕迹层本来就是为这种"真实但很薄"的日子设计的。
> 2. **《满12个月，81厘米24斤半》** → **撤销发布**（改 store_only），**且不要给 trace_eligible
>    标记**——按你自己 A-6 的规则，转发链接/带货内容本来就在排除类别里。
> 3. **3 条「可疑」**：同意你的判断不算「错」，但其中两条的补语（「妈妈估计是要换尿不湿了」、
>    「没哭，看着有精神」）同样是原文没有的推测，**顺手一并去掉**，成本很低。
>    《张小年被咬了》那条推理链合理、标题没有过度声称，保留不动。
>
> ### 🔴 关键防复发要求：重写文本必须先 predeclare，不许直接写库
>
> 重写本身也是写手在写，**同样可能再次脑补**。所以：把这 7 条（+2 条可疑）重写后的
> title/story 逐条贴进 STATUS.md，**跟它的原始证据并排列出来**，等我逐条比对确认后你再写库。
> 你 A-7 这次没动任何数据、老实 predeclare，这个做法是对的，继续保持。
>
> ---
>
> ## A-8 · 2026 的单源事件抽查（做完 A-7 的重写 predeclare 之后接着做）
>
> **为什么**：你证明了风险集中在"只有 1 条源消息"的事件上（36% vs 0%）。**2025 查完了，
> 2026 完全没查**——而 2026 是首页「最近」和张年页正在展示的内容，苏静打开网站第一眼就是它。
> 2026 有 353 条 life_events，全读不现实，但按你发现的模式可以精准打击。
>
> **目标**
> 1. 统计 2026 全年 approved life_events 里，**只有 1 条关联 raw_source** 的有多少条、占比多少。
> 2. 从这批单源事件里**随机抽 25 条**，按 A-7 同样的方法逐条对原文，给出错误率。
> 3. 如果错误率跟 2025 的 36% 同量级，把全部单源事件的清单（id + 标题 + 月份）导出来，
>    我来定要不要全量处理。
>
> **硬边界**：只读 + 抽查，**不改任何数据**，发现问题一律 predeclare。不改写手、不改阈值、不改 schema。
>
> **验收**：占比和错误率两个数字 + 25 条的逐条判定 + 单源事件清单（如果错误率高）。
>
> ---

> **📌 2026-09-06 03:30 UTC · A-6 验收通过，下一件是 A-7（见本文件稍下方 `## A-7`）**
>
> 重判我核对过了：167 → **153**，我点名的两条（「妈妈觉得张小年也已经不行了」
> 「妈妈和雪姨都觉得他不喜欢这个嫂子」）都已撤掉。**A-6 结案。**
>
> 另外告诉你一个结果：B 轨的痕迹层已经上线，你标的这些句子**现在真的出现在 2025 各月页上了**
> （我手机实看过 2025-06：「哄睡哄了将近半小时」「张小年今晚跟小雪睡」都在）。
> 你那一步直接变成了苏静会读到的东西。
>
> **接着做 `## A-7`：2025 全年 80 条已发布记忆的逐条核对。** 这是苏静读之前的最后一道内容闸。


> **⏰ 2026-09-06 03:05 UTC · 你已经 45 分钟没有写 STATUS 了，报个信。**
>
> 你最后一次写 `docs/STATUS.md` 是 02:16 UTC。按常设规则 5 分钟一次，现在超了 9 倍。
> 沉默 = 我判断不了你是在干活还是卡住了。**先写一行中间进度，再继续。**
>
> 提醒两件已经说过但你可能没读到的事（Teddy 03:00 前后叫三条轨都回读过入箱，如果你已经读了就忽略）：
>
> 1. **你的 push 没有卡住，不用等 Teddy。** 我直连问过真实远端：main = `5a8bc67`，
>    你那三个 commit 全是它的祖先，早就上去了（搭 B 轨 push 的车）。
> 2. **还没做完的是我 02:15 退回的第二件**：167 条按「只看标题这一行、不看 story」重判，
>    撤掉标题单独读会误导 / 吓人 / 暴露家庭摩擦的。我点名的两条是
>    「妈妈觉得张小年也已经不行了」和「妈妈和雪姨都觉得他不喜欢这个嫂子」。
>
> **另外一件新情况值得你知道**：B 轨发现痕迹层一条都没渲染出来，根因是
> `getStore()` 返回的 `store.events` 在 `assembleStore()` 里已经被过滤成发布态，
> 压根不含 store_only。B 已经改用 `getOrganizerStore()` 修好了（commit `2f65c78`）。
> 这跟你的 `life_event_trace` 隔离无关，你那部分是对的。
>
> 也就是说：**你标的那 167 条，等 B 的修复上线后就会真的出现在页面上**——
> 所以"按标题重判"这件事现在是**上线前的最后一道闸**，请务必做完再收工。


> **✅ 2026-09-06 02:50 UTC · 你的 push 没有卡住，不用等 Teddy，继续干。**
>
> 我用 `git ls-remote https://github.com/qianhaisu/Nian-Life-Journey.git main` 直连问了真实远端
> （不是本地那个可能过期的 `origin/main` 跟踪 ref——device_bash 没有 SSH host key，`git fetch`
> 会失败，所以本地 ref 不可信，这点记进你的坑清单）：**远端 main = `5a8bc67`**，
> 而你那三个 commit `77a1278` / `83b5d23` / `a1dd096` **全部是它的祖先，已经在远端了**。
>
> 原因：你自己的 `git push` 确实被分类器挡了，但 B 轨在 10:17–10:27 之间的几次 push 成功了，
> **push 会带上整条祖先链**，所以你的 commit 是搭 B 的车上去的。数据库写入本来就已完成，
> git 历史现在也同步了，**这件事不需要 Teddy 做任何操作**。
>
> 顺带表扬一句：你自己抓到的那个 `target_kind='life_event'` 会跟 `indexReviews()` 的键撞、
> 且那个查询没有 ORDER BY、可能非确定性地盖掉真实 store_only 判定——这个是真 bug，
> 而且是会静默让这 167 条从 B-17 渲染里消失的那种。抓得好。我之前只从文档层面
> 看到「文档和实现不一致」，没看到底下这层风险。
>
> **你现在继续做我 02:15 退回的第二件**：167 条按「只看标题这一行、不看 story」重判，
> 撤掉标题单独读会误导/吓人/暴露家庭摩擦的（我点名的两条：「妈妈觉得张小年也已经不行了」、
> 「妈妈和雪姨都觉得他不喜欢这个嫂子」）。第一件（改 STATUS.md 里给 B 的查询条件）如果还没改也一并改掉。


> **⚠️ 2026-09-06 02:15 UTC · A-6 验收结果：数据合格，但有两件事要你修。先修这两件，再收工。**
>
> 先说通过的部分（我独立查库核对过，不是看你的报告）：167 条已真实写进生产库，
> 按月 10/13/10/3/7/9/26/22/19/19/15/14 = 167，与你报的表**完全一致**；
> `target_id` 167/167 全部能对上 `life_events.id`；用独立 `prompt_version` 走唯一索引、
> 纯新增不覆盖，这个做法是对的。基准月 2025-06 收 9 排 3，我逐条看过，
> 「妈妈夸小年白得逆光都不怕」保住了，排掉的三条（奶奶说儿子有个幸福的家 / 提醒传照片到亲宝宝 /
> 游泳圈淘宝链接）理由都站得住。**判断质量本身是好的。**
>
> **要修的第一件（已经影响到 B 轨，我已经替你去 B 的入箱更正了）**：
> 你 STATUS.md 里写给 B 轨的查询条件是 `target_kind = 'life_event'`，
> **但你实际写进库的是 `target_kind = 'life_event_trace'`**。B 按你文档查会得到 0 行。
> 实现本身没问题（用独立 target_kind 反而更安全），**是文档写错了**——去把 STATUS.md 那段改对。
>
> **要修的第二件（真正的判断问题）**：痕迹层在页面上**只显示一行标题**，
> 但你是按 story 全文判「读起来会不会皱眉」的。抽读 20 条时我逮到两条标题单独看不成立的：
> - 「妈妈觉得张小年也已经不行了」——story 里说的是他老往一边歪，但标题单独拎出来读，
>   中文里「不行了」是很吓人的意思。这条如果出现在苏静翻的页面上，是事故。
> - 「妈妈和雪姨都觉得他不喜欢这个嫂子」——这属于你自己定的排除类别「家庭摩擦」，漏网了。
>
> **所以：把 167 条按「只看标题这一行、不看 story」重过一遍**，标题单独读会误导、会吓人、
> 或暴露家庭摩擦的，撤掉标记（删对应的 trace_eligible 行，或补一条撤销标记，你定，写清楚）。
> 撤掉多少条都行，宁可少不要错——这是「宁可没有，不要错的」的直接应用。
> 重过之后把新数字和撤掉的条目列表写进 STATUS.md，我再抽读一次。
>
> 不用重跑整个流程，只重判标题这一层。B 轨不等你，它已经先接上渲染了。


P1 全部结案。A-4（2025 回填）、A-5（4 个月 snapshot，结论是本来就不该有）都已完成。
**今天进 P2**，主题一句话：**让苏静能把 2025 年从头翻到尾**（P2 退出标准就是这个）。
今天三条轨并行，各自的活见各自入箱：A-6（本文，痕迹层数据）· B-17（月章节三层排版）· C-5（图片交付性能）。

**现在唯一要做的是下面的 `## A-6`。** 做完等 Cowork 抽读验收，不要顺手开别的。
worker 首次全量导入 Teddy 说先放着，**今天不要跑**。

---

## A-7 · 2025 全年 80 条已发布记忆逐条核对 — status: **ready，现在做**

**背景**　P2 的退出标准是「苏静能把 2025 年从头翻到尾，并说一句话」。她读到的「章节 / 段落」层
就是 2025 的 **80 条 approved life_events**（我查库核对过，年页上那个「80 段记忆」就是它们）：

`01:14  02:4  03:3  04:8  05:2  06:1  07:8  08:11  09:8  10:5  11:11  12:5`

A-4 抓到过一条真实误判——两只家猫的兽医体检报告被判成孩子去看兽医（猫也叫"年年"，跟他昵称撞了）。
**那是人工抽读发现的，不是系统挡住的**，而且根因至今没修（`subject-gate.ts` 遇到
「孤立昵称 + 转发的第三方聊天记录」这种组合仍可能误判）。同类问题很可能还藏在别的月份。

**80 条不多。这次逐条读完，不抽样。**「宁可没有，不要错的」——一条错的被苏静读到，
她对整个档案的信任就没了，那比页面稀疏严重得多。

**目标**　把这 80 条逐条读一遍，找出三类问题：
1. **主体误判**：写的其实不是张年（宠物、别人家孩子、撞名的人）。
2. **张冠李戴**：把转发的第三方聊天记录 / 科普文章 / 新闻当成他自己的事。
3. **事实与证据不符**：正文说的跟原始消息对不上——可疑的去查 `source_memory_links` 拉原文对照。

**硬边界**
1. **发现问题先在 STATUS.md predeclare，等我确认再动手删改。** 删 life_event 要走三张表
   （`source_memory_links` → `content_quality_reviews` → `life_events`），顺序见 HANDOFF-A 第 4 节。
2. **这次只核对，不改判官逻辑、不动阈值、不改 schema。** 收紧 subject-gate 是另一件事，
   要单独立项（我会在这轮之后按你的发现来定范围）。
3. 按月分批，每月写一段 STATUS.md 中间进度。

**验收（Cowork 抽读复核）**
1. 80 条全部有明确判定（好 / 可疑 / 错），可疑和错的要附原始消息对照。
2. 我随机抽 15 条复核你的判定。
3. **如果读完是零错误，也要明确写「零错误」**——那本身就是有价值的结论，不要为了显得有产出而挑刺。

**不可接受**
- 抽样了事（这次就是要全读）。
- 发现问题不 predeclare 直接删。
- 把「内容稀薄」当成问题报（薄是设计内的，错才是问题）。

---

## A-6 · 「痕迹层」数据：从 store_only 里挑出主体明确、可轻量展示的句子 — status: **✅ 已完成并验收（2026-09-06，167→153）**

**背景（Cowork 2026-09-06 亲自查库逐条读的，不是二手报告）**

2025-06 全月 13 条 life_events，只有 1 条 approved，其余 12 条 store_only。我把这 12 条全读了：
「妈妈夸小年白得逆光都不怕」「哄睡哄了将近半小时」「张小年今晚跟小雪睡」「雪姨说同事女儿发量比张年多」
「妈妈带张小年去小飞家」……**绝大多数是真的关于张年的日常，不是 not-about-child 误判**，
只是 T20-C 分级判为 low（不够「值得留下」）。
（更正一处：A-5 汇报里说这些是「含大量 low 级 not-about-child 行」——实查不成立，它们是 low 级 **about-child**。
以后写这类结论要抽读原文，不要只看分级字段推断。）

现在这些句子在页面上完全不出现，`/memory/2025/06` 只剩「1 段记忆 + 10 个光秃秃的空日期 + 折叠的 98 张照片」。
苏静翻到这个月会翻不下去——而 P2 的退出标准正是她能把 2025 从头翻到尾。

**产品判断（Cowork 定，不用重新讨论）**

月章节要分三层，不是「发布 / 消失」两态：
- **章节**：approved 且 worthiness 高 → 大版面，标题 + 正文 + 当天照片
- **段落**：approved 其余 → 中等版面（接近现状）
- **痕迹**：store_only 里**主体明确指向张年**的 → 一行短句，轻量呈现（怎么排是 B-17 的活）

这不违反原则五「生活不是等权的」——原则五要的是**用视觉权重表达差异**，
现在的实现把「权重差异」做成了「有和无」，是过度执行。
也正因为「宁可没有，不要错的」仍然成立，痕迹层必须有门——那就是你这一步。

**目标**

在**不放宽 approved 门槛、不改 T20-C 分级逻辑**的前提下，把 2025 全年的 store_only life_events
标出一个子集：主体明确指向张年、句子本身读得通、多年后读到不会让人皱眉的，标记为「可作为痕迹展示」。

**硬边界**

1. **不改 schema**：不加表、不加列。用现有 `content_quality_reviews`（同一 target 可多条 review）
   或 life_events 既有字段表达这个标记——你自己看 schema 决定，**在 STATUS.md 里写清楚你用了什么表示法**，
   B 轨要照着读。
2. **不把 store_only 改成 approved**。approved 是发布层；这次加的是展示层的另一个更低门槛，两者不能混。
3. **不删任何数据。**
4. 判断从严，拿不准就排除：主体不明确的（「奶奶说儿子有个幸福的家」这类关于家庭不关于张年的）、
   工具性内容（「妈妈提醒上传照片到亲宝宝」）、纯转述第三方的，一律排除。
5. 范围**先做 2025 全年**（12 个月）。2026 不动——2026 的 approved 密度够，不需要痕迹层。
6. 按月分批，每月跑完写一段 STATUS.md 中间进度。

**验收（Cowork 亲自抽读，通不过退回）**

1. 我随机抽 20 条你标为「可展示痕迹」的，**零条**不是关于张年的。
2. 我随机抽 20 条你排除的，排除理由要站得住（把「妈妈夸小年白得逆光都不怕」这种排除掉就是判错了）。
3. 每月给出数字：store_only 总数 / 标为可展示的数量，写进 STATUS.md。
4. **2025-06 单独列出全部 12 条的逐条判定和理由**——这是我验收的基准月。

**不可接受**

- 为了让页面丰满而把不确定的也标上（宁可没有，不要错的）。
- 顺手改 approved 判定、T20-C 分级阈值、主体门规则。
- 只报数字不给抽读样本。
- 跑 `nianlife-worker.mjs` 首次全量导入（Teddy 明确说先放着）。

---

## A-5 · 补 4 个月 monthly_snapshot（2025-02/03/05/06）— status: **✅ 已完成（2026-09-06，commit 1e8bd7c）：4 个月已发布事件只有 4/3/2/1 条，低于阈值，正常停在 quiet index，无需补**
...
对照决策 7（已定，不用重新讨论）：「整理与出版从最近月份做到最旧（2026 → 2025）」——2026 已经做完，2025 是排定的下一段，不是新决定，是执行排期到了。

**目标**　把 2025-01 到 2025-12 十二个月过一遍 T7 整理管线（跟 P1-0 处理 2026-01~05 用的是同一套流程），产出 life_events（有标题+正文）。

**顺序**　从最近往最旧：先 2025-12，一个月出 dry-run 样本给 Cowork 抽读、通过后再 `--commit`，再往前推 11、10……01。这是 T10 定下的既有流程（`organizer-month-write.mjs --day=YYYY-MM-DD` 或 `--from`/`--to`，指纹短路避免重跑已提交窗口），不用重新设计。

**硬边界**
- 主体门（P1-3，`gradeMonthEvents`）、信任名单制图文绑定（P1-4，`isTrustedPhotoSource`）、claim grounding 校验器全部沿用现有实现，不重新发明。
- **01 月内容阈值不降**（决策 8）——如果 2025 某个月素材本来就薄，输出薄属于正常，不要为了凑数放宽判官标准。
- 每处理完一个月，检查该月 `monthly_snapshot` 是否需要重新生成（现有 8 个月是 2026-01→08，2025 全年目前一个都没有）。
- 这是会花真实 DeepSeek API 时间和费用的工作（12 个月，每月可能要几十分钟到小时级），**不要一次性无脑跑满 12 个月不汇报**——按月出 dry-run，每月给 Cowork 一次抽读机会。

**验收**　Cowork 查库：2025 每月 life_events > 0（薄月除外，但要能看到判官日志说明为什么薄）；随机抽 2-3 个月在浏览器打开月页，确认渲染正常、图文配对符合 P1-4 规则；`monthly_snapshot` 新增对应月份。

**不可接受**　为了让每月都有内容而把主体门/信任名单标准放宽；一次性全跑不给中间检查点；跳过 dry-run 直接全月 commit。
