...
状态：queued | 关联：CMD-20260907-001 | 发送：Cowork | 接收：Code A | 类型：任务卡

**目标**：核查 Phase 2 第 18 节之后的实际进度、当前执行者和备份产物元数据；给总指挥和 Cowork 一份真实状态报告。

**允许修改路径**：`docs/STATUS.md`（出箱）、`docs/HANDOFF-A.md`（如需更新交接）。

**禁止操作**：
- 不连接 Neon 生产库（当前配额耗尽，连接会被拒绝，不要试图重连或绕开）
- 不重新索取凭据（复用已有授权入口或记录阻塞原因）
- 不盲目重试（按总指挥指令：两次信号中断后停止在该 VM 内重试）
- 不启动新备份进程（NIANLIFE-RUN-FULL.bat 等待总指挥下次明确放行后在执行）

**前置依赖**：无（只读，不依赖其他轨）

**验收方法**：在 `docs/STATUS.md` 写出以下三项：
1. Phase 2 实际产物：`C:\Users\teddy\Downloads\` 和 `E:\NianlifeBackups\2026-09-07\` 下的文件清单及大小（如果这些目录可读）
2. 第 18 节之后有无额外执行记录（读 `docs/nianlife-P2-backup-execution-2026-09-07.md` 第 18 节后是否有新内容）
3. 当前阻塞：生产连接恢复需要什么条件

**资源占用**：只读文件系统，不运行数据库进程，不消耗网络。

**收到此任务请在 `docs/STATUS.md` 写 ACK 一行，格式：`MIG-A-001 ACK YYYY-MM-DD HH:MM CST`**
...
> `title`/`story` 已经是「张小年早上把头摔了」/「妈妈说，张小年早上把头摔了。」，跟你报的
> 一致。A-9 到此确认结案，不用再回头看。
>
> **「孤立昵称+第三方转发聊天记录」范围看过了，批准，可以开工**：两阶段（SQL 候选 + 人工
> 核对）思路对，`life_events` 的 `approved`+`store_only` 两层都要看这个判断也对（因为
> `store_only` 会喂进痕迹层，苏静一样能读到）。不需要扩大到 `daily_traces`——你拟的
> `life_events` 两层已经是喂进痕迹层的完整来源，`daily_traces` 是更底层的原始痕迹，不是
> 苏静会直接读到的展示层，这次任务不用管。也不需要额外去重扫 A-6 的 153 条痕迹层标记——
> 那 153 条本来就是从这次要扫的 `life_events` 里派生出来的，扫源头就够了，不用扫两遍。
> **规则本身（`subject-gate.ts` 的 `SUBJECT_NAMES`）先不动**，跟你拟的一样，只找现有数据
> 里已经产生的误判，不改判官逻辑。
>
> 按你拟的验收方式来：候选清单数量 + 阶段二逐条判定表（跟 A-7 一个格式）+ 如果有真误判，
> predeclare 等确认再删改，不要自己动手删。开工吧。
>
> 撤销发布那条查过了——`quality-review-70b40088-...` 现在是 `store_only`，没打
> `trace_eligible`，文本没动，跟 predeclare 一致，这条彻底结案。
>
> **「脑门起了个包」：保持 `approved`，不降级、不加 trace_eligible，用你给的第一个版本
> 「张小年早上把头摔了」。** 理由：去掉编造细节之后剩下的是一句站得住的事实，跟决策 8
> （内容薄不放宽标准、原样展示）是一回事——单薄不等于不该发布，发布的东西必须是真的
> 才是底线。不用因为比别的条目单薄就降级，除非以后专门有一轮"薄内容分层"的任务。
> predeclare 通过，写库；写完照例回报影响的行数（这次应该是纯 `life_events` 文本 UPDATE，
> `content_quality_reviews.decision` 不用动，本来就是 approved 不变）。
>
> **写完这条，A-9 彻底结案。** 接着做排队的：全库"孤立昵称+第三方转发聊天记录"模式扫描
> （A-4 收尾时你自己提的那个，任务范围你先拟一版给我看再动手——这个可能会碰到发布状态，
> 按老规矩 predeclare）。

<details><summary>历史：A-9 收尾确认 + A-10 验收通过（已执行完毕，见上）</summary>

> ## 📍（历史）2026-09-06 12:5x UTC：**A-9 收尾确认 + A-10 验收通过**
>
> **A-10：代码级验收通过。** 直接读了 `lib/db/postgres-repository.ts:473`，`reviewFromRow`
> 现在就是 `row as unknown as QualityReview` 的纯透传，`normalizeQualityDecision` 确实不再
> 挂在读取层。106/106 测试通过 + 这处最小改动，加上 B 轨昨晚已经用生产 fetch 验证过 A-6 的
> 153 条 trace_eligible 子集正确显示（B-17 验收第 1 条），两边对得上。**你说的 `getStore()`
> 环境挂起不用再深挖**，不属于 A-10 范围，代码 + 现有测试 + B 的生产验证已经足够，不需要
> 再跑那个脚本。**A-10 结案。**
>
> **A-9 「满12个月，81厘米24斤半」撤销发布：确认，照你的 predeclare 执行。**
> `UPDATE content_quality_reviews SET decision='store_only' WHERE id='70b40088-96b4-44a1-ae28-bc4d7c45f4eb'`，
> 不加 trace_eligible 标记，不改 `life_events` 文本。写完库回报一下实际影响的行数。
>
> **「脑门起了个包」`event-v2-496fde6db412f3b9b09bbe20171908b9`：你判断得对，这条确实不该被
> 拍板覆盖。** 没有配图、文字证据也只到"摔了一跤"，没到"起包"这个具体细节，属于决策 16 里
> 明确排除的那一类（完全没配图 + 文字不支持的具体断言）。**不是撤销发布，是把"起了个包"这个
> 无凭据的细节去掉，只保留文字能撑住的部分**——具体怎么改标题/正文你先按 A-7/A-8 那种三段
> 并排（原始证据 / 现文 / 改后）predeclare 一下给我看，我确认后你再写库。
>
> **防复发方案（"单源 + 无配图才收紧写手扩写"）：方向同意，先不实现。** 这是对的收紧范围——
> 不是所有单源事件都要管，只在"文字和图片都薄"这个交集上收紧。但这次不动 `lib/organizer/**`
> 代码，等专门立项时再做，你现在只要把这个方案写进 STATUS 存档就行，不用现在写代码。
>
> 上面两件事（撤销发布 1 条 + "脑门起了个包" predeclare）做完、A-9 才算真正结案。
> 结案后如果入箱没有新任务，正常进入空闲回读；下一单大概率是全库"孤立昵称+第三方转发聊天
> 记录"模式扫描（A-4 收尾时提过的那个），或者 P1-6 全量导入，等我这边排。

</details>

<details><summary>历史：2026-09-06 04:45 UTC Teddy 拍板（已执行完毕，见上）</summary>
