...
**更新于 2026-09-05 00:0x（Claude Code）· T20-B 的 bug 已修复并核实真的落库了；跑题记忆已修正；下一件 T20-C+T19**

| # | 任务 | 在第几节 | 状态 |
|---|---|---|---|
| 1 | **2026-08 整月 T7** | `### 🎯 P0 验收范围` | ✅ 完成（46 条 life_event，0 家人，全部有标题） |
| 2 | **T18 照片落库 + 首页「最近的一天」** | `### 🐛 T18` | ✅ **Cowork 22:5x 已验收通过**；backfill 对 06/07/08/09 全部跑过 |
| 3 | **T21 撤掉月首照片计数** | `### 🐛 T21` | ✅ 完成并推送 `5e5a995`，Cowork 23:3x 已 curl 四个月页核实 |
| 4 | **T20-A** | `### 📐 T20` A | 🟡 A1/A2/A4 已验收通过；**A3（月末档案封顶）未做** |
| 5 | **T20-B 月度回顾**（原则七） | `### 📐 T20` B | ✅ **这次是真的**——之前"完成"是假的（脚本忘设 `REPOSITORY_BACKEND=postgres`，三次 `--commit` 全写进本地 JSON 文件，Cowork 23:4x 抓到）。已修复脚本 + 重新对 06/07/08 跑 `--commit` + 直接查生产库确认 3 行都在。**求 Cowork 重新验收** |
| 6 | **一条跑题记忆已修正** | — | ✅ `event-v2-a78ad09fdfcc339bb4ecfa016207a5ef`（讲这个网站项目自己 GitHub 协作的，跟张年无关）已把 review 决定改成 `rejected_unrelated`，不再发布；行本身未删。全库关键词扫描没找到第二条同类 |
| 7 | **T20-C 记忆分量 + T19 群务降级**（原则五）| `### 📐 T20` C | 🔴 未开始，下一件；Cowork 23:3x 已给出 08 月约 20/46 条具体清单 |
| 8 | T15-D `npm test`（643/643 通过，已做）；T15-E 孤儿 run；T20-D /about 逗号 | — | 🟡 D 完成，E/D 未做 |

✅ 已完成：T17 并发、T15-A、**T15-B 乳儿班 7,244 一条不差**、2026-06（32）、2026-07（46）、2026-08（46）、2026-09（4）、
**T18**、**T20-A1/A2/A4**、**T21**、**T20-B（真的落库了）**、**跑题记忆修正**。**P0 四个月原始数据 + 文字 + 照片绑定 + 月度回顾全部到位。**
