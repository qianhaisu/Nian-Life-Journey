...
2. **没做到 / 最大 blocker**：**配图恢复样本卡在只读数据**。8 月 18 篇故事的正文与证据链已逐篇看过、
   ...
   只用于收窄候选，不单独上线作为配图规则。**
   已确定的一条：**全仓库没有任何代码写入过 `NO_HERO_MEDIA_ID`**，所以 `hero_media_id='none'` 只可能是
   人工明确撤图；2026-09-10 的批量隐藏改的是读取侧 `isStoryAssociated`，一行 `hero_media_id` 都没动过。
   两种状态在数据里干净可分，不能把 noPhoto 一概当人工决定，也不批量清除。
