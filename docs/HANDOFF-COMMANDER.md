...
### 2026-09-10 三续：2a13451 选图规则 + 照片区验收

容器已换到 `nianlife-web:2a13451f604741598dca88ca7a9ad15340e57a50`，image id `d42e012a8a25` → **`d2b67453eb44`**。
换前与同机 session `nianlife-7d` 确认无占用；RDS、18080、DNS 均未动，未公开部署。详情见
`docs/STATE.md` 顶部「一之负一」。

- **通过**：无可靠关联的故事不再借图（18 个 memory_led moment 合计 0 图，08-19 月页与详情页均 0 图，
  noPhoto 仍生效）；手机 390×844 下照片入口在首屏内、点击准确落到照片区、展开后 30 个日期各带标题；
  此前撤出的三张照片按 ID 在「8 月 19 日」组里找到并能打开（200 / image-webp / 真实字节）；
  懒加载成立（661 张全 lazy，不一次性加载）；零横向溢出；首页与归档索引首次响应仍无 mock。
- **未能验收，需要人工决定**：唯一有可靠关联的故事「假哭时睁眼偷看有没有人哄」(2025-08-29)
  质量审阅决定是 `needs_review`，**从未发布**，页面 404。因此已发布的 212 个故事里，
  能渲染配图的从 182 变成 **0**，站上现在没有任何故事配图。发布与否是人工审阅的事，本轮未动。
- **新发现，需要判断**：照片区默认展开后，聊天流截图跟着显眼了。2026-08 的 664 张可绘制图片里
  **115 张来自不可信会话**，含银行转账与住房公积金账户明细。一直在档案层，只是以前折叠。
  收敛方向是让照片区沿用 photo_led 的来源可信过滤，属产品判断，未擅自改。
- **口径订正**：「30 天 640 张」是档案层可绘制 664 张减首屏 24 张；「29 天 549 张」是另一套
  （仅照片 + 主图尺寸下限 + 来源可信）。没有发现任何实际不可访问的照片。
- **仍在的阻塞**：worker 的 revalidate 碰不到首页与归档索引（恢复 worker 前必须修）；视频未交付。
