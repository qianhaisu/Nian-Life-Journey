...
## 🔶 2026-09-06 03:15 UTC · B-17 第一轮验收：痕迹层成立，但退回 6 条（第 1 条最急）

**先说成立的**：修复已上线，我手机 375 实看 `/memory/2025/06`，痕迹层真的出来了——
6/1「哄睡哄了将近半小时」、6/3「张小年今晚跟小雪睡」、6/4「妈妈不愿小年再撞伤」……
**那些日子不再是光秃秃的日期了**。这一层的机制是对的，根因也找得准（`assembleStore()`
过滤成发布态那条，我只猜到方向，你定位到了具体位置）。

**但离 B-17 验收标准还差得多，按优先级退回：**

**1. 🔴 最急：线上现在就有不该出现的内容。** 你还在用 `store_only` 全集，
而 A-6 的子集已经就绪（153 条，`target_kind='life_event_trace'` + `decision='trace_eligible'`）。
2025-06 现在线上渲染出来的这三条，是 A 逐条读过、**明确判掉**的：
- 「奶奶说儿子有个幸福的家」——主体是家庭不是张年
- 「妈妈提醒上传照片到亲宝宝」——工具性提醒
- 「妈妈问雪姨新游泳圈会不会好点」——购物链接分享

**立刻切到 A-6 子集**（你当初就是按"抽成一个函数方便一行切换"写的，现在就是切的时候）。
这条违反「宁可没有，不要错的」，优先于其他所有改动。

**2. 🔴 "一天一条"的去重静默丢掉了好东西。** 2025-06 有 12 条 store_only，页面只出现 10 条，
被挤掉的两条里包括 **「妈妈夸小年白得逆光都不怕」**——这句从头到尾是我用来定义"痕迹层为什么值得存在"
的基准句。同日多条时不能静默丢弃：要么都显示（一天多行），要么按 worthiness/时间排序取优，
但**不能让好句子因为撞了日子就消失**。切到 A-6 子集后同日冲突会变少，但规则本身要改。

**3. 空日期列表还在。** 6/8、6/13、6/14、6/18、6/21、6/28、6/30 仍然是**只有日期、没有任何内容**。
入箱验收标准第 1 条写的是「**不再有光秃秃的空日期列表**」。现在的效果是"部分日期填上了一行字，
其余照旧空着"——结构没变。这些日子有照片（见第 4 条），照片就是它们的内容。

**4. 照片一张都没进正文。** 「整月照片档案 98 张」这个计数式标题还在原位，98 张照片仍然全折在里面。
验收标准第 1 条要求「照片出现在正文里」，第 4 条要求去掉计数式表述（原则三）。**这是 B-17 的主体工作，
也是苏静最在意的东西——她要看的是照片。** 目前这部分一行代码都还没动。

**5. 手机 375 上有一大块空白。** 「体检抽血没哭」那段结束之后到「这个月的日子」标题之间，
约 800px 纯空白（滚一屏半什么都没有）。我查过 HTML：两者之间只隔 216 字符，**没有 img、没有 figure**，
所以不是图片没加载，是 CSS 间距问题（`section.month-days` 或前一个 moment 的上边距）。

**6. 文案自相矛盾。** 页面顶部写「这个月记下 1 天。」，但下面实际列出了 10 天有内容。
痕迹层上线后这个计数口径要跟着改。

**下一步怎么排**：1、2 先做（内容正确性，线上带着错的东西），做完立刻 push，我复验。
然后 3、4 一起做（它们是同一件事：让有照片的日子用照片做内容，空日期自然消失）。5、6 顺手。

**每 5 分钟写 STATUS-B.md 之前先回读本文件顶部**（新常设规则，见上）。
...
| 页面 | 整月照片档案(旧) | 这个月的日子(旧) | moment-trace | css hash |
|---|---|---|---|---|
| /memory/2025/06 | 有 | 有 | 0 | 5511962116b59e3a |
| /memory/2026/08 | 有 | 有 | 0 | 5511962116b59e3a |
| /memory/2025/12 | 有 | 有 | 0 | 5511962116b59e3a |

三个页面都是 `PRERENDER`/`HIT`、age 很小，说明是当前生产构建**新鲜渲染**出来的——
**结论：线上跑的确实还是 B-17 之前的构建，你的改动没上线。** 你等了 30 分钟，判断没错，
但**继续轮询页面是浪费**：Vercel 那边我够不到（device_bash 没有 vercel CLI，也不该碰它的 token），
已经上报 Teddy 去看 Vercel 控制台有没有 failed / stuck 的构建。

**你现在改做这三件，不要空等：**

1. **本地起服务验证**（`npm run dev` 或 `npm run build && npm run start`），用本地
   `/memory/2025/06`、`/memory/2025/12`、`/memory/2026/08` 走一遍 B-17 入箱里那四条验收标准。
   本地验证过的东西，等 Vercel 一通就能直接上线，不会白等。
2. **必看：痕迹层的查询条件**（我在上一条急件里写过，如果你还没读——A 轨文档写的是
   `target_kind = 'life_event'`，**实际是 `'life_event_trace'`**）。用错的话痕迹层会是空的，
   而且看起来像你自己的 bug。本地验证时**先确认 2025-06 那 9 条痕迹真的渲染出来了**
   （2025 全年应该是 167 条，按月 10/13/10/3/7/9/26/22/19/19/15/14）。
3. 本地跑通后在 STATUS-B.md 写清楚：本地验收结果 + 截图/结构描述，标明「等部署」。
   Vercel 恢复后我会立刻做线上视觉验收。

**不要**为了触发部署去追加空 commit 或反复 push——问题不在你这边。
...
今天三条轨并行：A-6（痕迹层数据）· **B-17（本文，月章节三层排版——今天的主战场）** · C-5（图片交付性能）。

**只看下面的 `## B-17`。** 做完等 Cowork 手机验收，不要顺手开别的。

---

## B-17 · 月章节三层排版：让 2025 每个月都能翻下去 — status: **ready，现在做**

**为什么现在做这个（Cowork 今天用手机 375 宽实际翻过 2025 各月）**

问题不在内容量，在排版形态。`https://nianlife.cn/memory/2025/06` 现在是这样：

```
这个月记下 1 天。
这个月记下来的  →  6 月 5 日 · 体检抽血没哭（就这 1 段）
这个月的日子    →  6月8日 6月9日 6月13日 6月14日 …（10 个光秃秃的日期，没有任何内容）
这个月还有 15 天留下了零散的照片，收在下面的档案里。
整月照片档案 98 张
```

一个月 98 张照片全被折叠进「档案」，正文主体是十个空日期。**这同时违反两条产品原则**
（`docs/nianlife-product-principles.md`，动手前先读）：
- **原则三 Media First**：「用『X 张照片 / Y 条消息』代替内容本身」是原文明确列出的违反方式。
- **原则七**：材料不足可以只做安静的索引，但索引不该是一串没有内容的日期——
  「一份回顾去掉所有数字后，是否仍然能读出这个月的张年」，现在读不出。

丰富月份同样中招：`/memory/2026/08` 有 664 张照片，也全在折叠档案里。

**目标**

月章节从「一坨文字 + 空日期 + 折叠档案」变成**可以从头翻到尾的一章**，四件事：

1. **三层视觉权重**（对应原则五「生活不是等权的」）：
   - **章节**：approved 且 worthiness 高 → 大标题 + 正文 + 当天照片，占大版面
   - **段落**：approved 其余 → 中等版面（接近现状）
   - **痕迹**：store_only 里主体明确的子集 → 一行短句，浅、轻、密，不占大版面
2. **消灭「这个月的日子」那串空日期。** 只有照片没有文字的日子，让**照片直接进正文时间流**
   （配日期 + 当时年龄，原则二 Two Clocks），不再降级成一个光秃秃的日期。
3. **照片不再只有「折叠档案」一个出口。** 档案入口可以保留在章节末尾，但它不能是这个月照片的唯一去处。
4. **去掉计数式表述**（原则三）：「整月照片档案 98 张」这种不要再出现在正文主体位置。

**硬边界**

1. **不碰 `v2/lib/organizer/**`、`v2/lib/db/**`、`v2/scripts/**`、`v2/.env.local`**——A 轨今天在里面干活。
2. 照片筛选规则一行不改：`isPortraitOfZhangnian`（`lib/media/representative.ts`，B-15 定的）
   封面/卡片/cluster 只认夸克；**月页正文当天照片不受此限**（决策 14 原文写了），沿用现状。
3. **A-6 的痕迹层标记今天可能还没落地。** 你先用 `store_only` 全集把排版做出来（视觉先成立），
   A-6 落地后把数据源从「全集」换成「标记子集」是一行的事。**不要因为等 A-6 停工**，
   但要把数据源抽成一个函数，方便一行切换。
4. **图片性能由 C-5 并行处理，你不要自己改 `/api/media`**（C 轨领土，今天在改）。
   正文图片数量会显著增加：一律用 `variant="thumbnail"`，等 C-5 在 `docs/STATUS-C.md` 给出
   variant / 尺寸档位结论后照它的结论调。
5. 上线前 `npm run typecheck` 通过；`git add` 只加自己改的文件（`git add -A` 会混进 ~260 个 CRLF 假改动）。

**验收（Cowork 手机 375 + 桌面亲自翻，通不过退回）**

1. `/memory/2025/06`（最稀薄：1 段记忆 + 98 张照片）：翻起来像一个月的生活，
   **不再有光秃秃的空日期列表**，照片出现在正文里。
2. `/memory/2025/12`（最丰富：359 张照片 + 完整月度回顾）：三层权重一眼能分辨——
   「小年年迈出独立走路的第一步」明显比「十天用掉大半，还拉了」重。
3. `/memory/2026/08` **不退化**：月度回顾、逐日记忆、照片都还在，只是排版升级。
4. 手机 375 宽：无横向滚动、竖图不被拉伸变形（B-16 的 `fit` prop 规则继续生效）。

**不可接受**

- 为了塞进照片而把不属于当天的照片放进当天。
- 把「整月照片档案」原样留着、旁边再加一块新的——那是加法，不是重构。
- 只在桌面验证就报完成（**苏静在手机上看**）。
- 越界改 A 轨或 C 轨的文件。
...
> **2026-09-05 15:56 UTC Cowork：B-16 复验通过（部署确认 + 手机 375 实测卡片全部 160px/photo-crop，月页正文按自身比例未被误伤）。B 轨这轮（B-1~B-16）全部结案。入箱暂无新 ready 任务——不用空转硬找事做，等 Teddy 或 Cowork 派新任务时再继续；如果要 /clear，先照五段格式把 `docs/HANDOFF-B.md` 覆盖写一次「现在」的状态。**
