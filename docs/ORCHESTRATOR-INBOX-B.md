...
## PAGE-0915-HOME-BALANCE · 首页竖图版式回调（立即执行）

状态：ready | 发送：Codex 总指挥 | 接收：现有页面 Code section `959e524a-d044-45fd-ab63-36e287253052` | 时间：2026-09-15 CST。

### 用户判断

当前桌面首页在竖图轮播项上失衡：右侧竖图像一堵接近通高的墙，左侧标题与提醒被 `justify-content: space-between` 拆到上下角落；18–21px 常规字重的题签在宽屏上弱得像加载异常。目标不是换内容，而是恢复「聚集与平衡」：左侧日期、标题、入口、这几天的事成为一个视觉块，整体与照片垂直居中；竖图像墙上的画，四周有呼吸感。

### 只做这些

1. 仅调整首页展示层，优先范围 `v2/app/home.css`；只有确有必要才动 `v2/components/home-lead.tsx`。不得改 `home-feed`、轮播候选、配图审核、提醒筛选、标题强调、照片点击去处、视频或数据库逻辑。
2. 桌面 ≥900px：`.home-aside` 不再把题签与 `.home-notes` 推到上下两端；它们保持同一列自然流，内部间距约 56–72px，整个组合与右侧照片主体垂直居中。照片 caption 不应改变左块的视觉中心。
3. 桌面标题恢复到上一版有分量的题签：建议 `clamp(30px, 2.5vw, 36px)`、500 左右字重、紧凑但可读的行高；仍最多两行，保留现有强调色与真实标题，不改文案。
4. 竖图专门限高：根据图片固有宽高或等价 CSS 方案，对 portrait 使用约 `75dvh` 的最大高度（并给出合理像素上限），保持原比例、不裁切、不拉伸、水平居中或靠右但四周留白。横图不要被竖图规则缩成小图，继续充分使用右栏宽度。
5. 手机维持现有阅读顺序：照片 → 日期/年龄 → 标题/入口 → 提醒；不得产生横向溢出，点击照片仍进入对应当天故事，换图仍连带切换标题、日期、年龄与链接。

### 验收与交付

- 先确认当前 `main`、占用板和工作区他人改动；只精确暂存自己的文件，不碰现有 `CLAUDE.md`、`HANDOFF-COMMANDER.md`、`ORCHESTRATOR-INBOX.md` 与未跟踪旧文件。
- 覆盖至少一个竖图 slide 和一个横图 slide；在 1440×900、390×844 分别实际打开确认。桌面竖图不得接近顶天立地，左侧四项形成连续视觉块且整体居中；横图仍有足够视觉体量；手机零溢出。
- 运行相关测试、typecheck、lint、生产 build；通过后提交并 push 当前 `main`，不建分支/worktree。
- 私有部署已有站内授权：审查通过后可部署到既有私有诊断容器，保留回滚点；随后实际打开 `127.0.0.1:18080` 与对应私有站，各做 1440/390 验收。不得公开切换、改 DNS、启动 Organizer/worker 或重跑生产数据。
- 回报：完成内容、commit SHA、测试、两种方向照片与双端实看证据、回滚点、阻塞、下一步。完成前不要接别的页面任务。
...
状态：queued | 关联：CMD-20260907-001 | 发送：Cowork | 接收：Code B | 类型：任务卡

**目标**：仅离线建立迁移后页面/产品八原则验收清单，记录当前不能在线验证的项目。

**允许修改路径**：新建 `docs/migration-B-acceptance.md`（唯一产出）。不改任何代码。

**禁止操作**：
- 不访问 nianlife.cn（站点当前不可用）
- 不改任何 UI 代码、组件或样式
- 不推断当前线上状态（用最后已知代码状态作为基线）

**前置依赖**：读 `docs/nianlife-product-principles.md`（产品八原则，必读）

**交付**：`docs/migration-B-acceptance.md`，包含：
- 产品八原则逐条验收标准（迁移后应达到什么状态）
- 当前基线状态（B-17、B-18 已落地，P1/P2 完成情况）
- 无法在线验证的项目（注明原因：站点不可用、Neon 配额耗尽）
- 上线前 B 轨需补的任何 UI 检查点

**验收方法**：Cowork 读 `docs/migration-B-acceptance.md`，确认逐条有依据，未验证项目有明确原因标注。

**资源占用**：只读代码库，无网络需求。

**收到此任务请在 `docs/STATUS-B.md` 写 ACK 一行，格式：`MIG-B-001 ACK YYYY-MM-DD HH:MM CST`**
...
> ## 📍 当前任务（2026-09-06 12:5x UTC 更新，B 轨 05:2x UTC 标记完成）：**B-18 — 状态：✅ 已上线并验证**
>
> commit `776fa67` 已 push main 并部署（Ready，alias 到 `nianlife.cn`）。浏览器直连生产
> `/memory/2025/06` 抽查全部 45 张 `main img`：0 张走 `/_next/image`，45 张全部直连
> `/api/media/...`。已在 `docs/ORCHESTRATOR-INBOX-C.md` 给 C 轨留言，C 可以重跑 C-6 预热。
> 详见 `docs/STATUS-B.md` 最新两条。B-18 结案，等下一单或进入空闲回读。
>
> B-17 已结案（见下方历史）。原派单文字保留：**B-18：给媒体图片加 `unoptimized`，解决 C-6 卡住的路线问题。**
>
> **背景**：C 轨 C-6（预热读图路径）卡在一个问题——页面上的图有两种 URL 形式，
> 原始 `/api/media/X?variant=thumbnail` 和 Next 图片优化器包装过的
> `/_next/image?url=%2Fapi%2Fmedia%2FX...&w=...&q=...`，这是两个不同的 CDN 缓存键，
> C 预热一个不会让另一个变热。C-5 已经查清楚：这些图是**入库时就生成好的定宽 webp**
> （`thumbnail`/`web` variant），不是原图，Next 优化器对它们做的"响应式裁切/转码"本来就是
> 白做一遍——STATE.md 踩坑记录里也写过"夸克 web 变体经 Next 优化器超时"这个问题。
>
> **目标**：`v2/components/photo.tsx`、`photo-viewer.tsx`、`evidence-list.tsx` 这三处
> `next/image` 的 `<Image>` 加 `unoptimized`（或换成你判断更合适的等效做法，比如原生 `<img>`——
> 你更懂这几个组件，方式你定，但**验收标准不变**：改完之后，线上任何一张走这三个组件的图，
> 浏览器实际请求的 URL 必须是 `/api/media/...` 原始形式，不能再出现 `/_next/image?url=...`。
>
> **硬边界**：
> - 不改这三个文件之外的东西；`variant`/`fit` 的选择逻辑不动，只动"要不要走 Next 优化器"这一层。
> - 加完之后自己开发者工具网络面板（或生产 fetch）抽查几张确认 URL 形式变了，再 push。
> - `failed` 兜底的 `<img>` fallback（`photo.tsx` 里已经是原生 img）不用动，它本来就没走优化器。
>
> **完成后**：出箱写清楚改了什么、抽查了哪几张图确认 URL 形式，然后**在 INBOX-C 里给 C 轨留一句
> "B-18 已上线，commit <hash>"**（跨文件写这一句可以，其余部分不要碰 INBOX-C），C 会接着重跑
> C-6 预热和对照实验。做完 B-18，入箱若无新任务就正常进入空闲回读。

<details><summary>B-17（已结案）历史记录</summary>

> ## 📍 当前任务（2026-09-06 12:5x UTC 更新，Cowork 复验通过）：**B-17 — 状态：✅ 已结案**
>
> Cowork 用真实浏览器复验了 `/memory/2025/06`（同日多条不丢的那个月）：
> **手机 375px**（真实截图，非 DOM 推断）——6/4「妈妈夸小年白得逆光都不怕」正常显示在 6/5
> 之前不丢；6/8、6/13、6/14 这些原来的"空日期"现在都有照片直接进正文，滚动一路下去没有
> 任何空白断层。**桌面宽度**（768px 截图 + 全文档 `getBoundingClientRect` 结构扫描，13333px
> 高度、每个 `.month-day` 高度都对应真实图文内容）——同样没有异常空白。原第 5 条「~800px
> 空白」问题确认已消失，不是碰巧漏检。**B-17 结案，可以 /clear。**
>
> （复验过程中撞见一个纯工具 bug，跟你的代码无关：这次浏览器的 `resize_window` 切到自定义
> 1440x900 后再滚动截图会拿到一张黑白拼帧的假"空白"截图，DOM 层 `elementFromPoint` 一查其实
> 内容都在。换成 tablet 768 预设或刷新后就正常——记录一下，免得以后谁看见类似的"空白截图"
> 又误判成产品 bug。）
>
> **不要用"时间戳比我这次启动新"来判断有没有活干**——你这次是重启的 session，
> 入箱里所有东西都比你启动早，那样判断永远得出"无新任务"，今天已经真实发生了
> （你 03:58 报"no new inbox instructions"，但 03:15 那块 6 条退回一直躺在下面没人做）。
>
> **正确判断方式：读这一行。它说要做什么就做什么，直到它被标成 ✅。**
>
> **现在要做的是下方 `## 🔶 2026-09-06 03:15 UTC · B-17 第一轮验收` 那一整块的 6 条退回。**
> 最急的是第 1 条：**切到 A-6 的 153 条子集**（`target_kind='life_event_trace'` +
> `decision='trace_eligible'`）——线上此刻仍在显示 A 轨明确判掉的三条内容
> （奶奶说儿子有个幸福的家 / 妈妈提醒上传照片到亲宝宝 / 妈妈问雪姨新游泳圈会不会好点）。
> 做完这条就 push，我立刻复验，不用等其余 5 条一起。
>
> 全部 6 条做完后把本行状态改成 ✅ 并在出箱写完成汇报。
