...
- 媒体交付方式已由总指挥定为：**私有 OSS，由 ECS 经同地域内网读取并受控输出**。复用 ECS 固定公网带宽控制公网流量支出；OSS 仍待 Phase 2 备份验证后再购买。
- Phase 2（源系统冻结与备份）：**执行方案已成文并经总指挥审核批准（`docs/nianlife-P2-backup-execution-2026-09-07.md`）；当前电脑尚未安装 PostgreSQL 18 工具，先完成本地工具与隔离恢复环境准备，再执行生产只读备份**。
...
1. Claude Code 按已批准的 `docs/nianlife-P2-backup-execution-2026-09-07.md`，先准备 PostgreSQL 18 本地工具和两个隔离恢复库，再执行 Neon 只读备份与恢复验证。
2. P1-01 备案资料已填写并完成身份证/人脸核验；当前等待苏静录制网站负责人承诺视频，上传后重新提交初审。
3. Claude Code 按“私有 OSS → ECS 内网读取 → ECS 受控输出”的既定方式设计 C4，范围包含 `hot-storage.ts` 之外的相关 Quark/HEIC 脚本；本阶段只做设计，不购买 OSS、不开始代码改造。
4. Phase 2 备份执行、验证通过后，再在 ECS 上开始容器化/发布流程等新开发项（第 4 节所列）。
5. 迁移期间静态可用方案（第 8 节）如果 Teddy 决定推进，按第 10 节任务卡边界执行（只比较不部署）。
6. `a04d8d2`/`94ea000` 已随 `main` 同步到 GitHub；Vercel Git 连接已断开，因此本次 push 未触发构建或部署。这两个修复仍需在后续技术验收中验证，不能因已推送就视为已上线。
...
|---|---|---|---|---|---|---|---|
| P1-01 | 备案资质提交（苏静个人，浙江省，首次备案，网站名称"岁月拾光册"） | Teddy／苏静 | 身份/域名合规性已确认满足（见第 9 节） | 在阿里云备案系统内自行填写主体信息、证件、联系方式；本 Session 不参与、不留存材料 | 备案申请材料（不进入 Claude 文档或仓库） | 备案系统受理，进入初审 | **身份证/人脸核验已完成；等待苏静录制承诺视频并重新提交初审** |
...
| P1-05 | OSS 采购与媒体交付方式选型（暂不购买） | 总指挥定稿；Teddy 后续采购 | ECS 已采购 | 仅确定架构，不购买 OSS | 私有 OSS + ECS 内网读取并受控输出 | 交付方式明确，公网传输复用 ECS 固定带宽 | **选型完成；OSS 暂不购买** |
...
| P1-07 | Phase 2 数据备份执行 | Claude Code | **总指挥已审核批准 `docs/nianlife-P2-backup-execution-2026-09-07.md`** | 先准备本地 PostgreSQL 18 工具与隔离恢复库，再按方案只读连接 Neon | 两份验证过的备份 + 核对报告（存放于仓库之外的受控目录） | 见执行方案文档验收标准 | **已批准；待工具准备与执行** |
