...
状态：**凭据已按建议轮换。使用轮换后的新连接串重试两次（一次普通方式、一次加 setsid 隔离终端会话），中转文件均按协议正确验证并立即清零，连通性检查均确认为真实生产连接、库规模正常，但两次 `pg_dump` 全量备份均在启动后几乎立即被信号中断（`terminated by user`），零数据导出。按总指挥事先约定的止损条件，本轮到此为止，不在这个 Cowork 桥接 VM 内继续重试，详见第 9 节。**
