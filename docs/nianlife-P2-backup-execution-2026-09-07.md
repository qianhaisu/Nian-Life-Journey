...
**当前阻塞、需要 Teddy／总指挥决定的两件事**：
1. **连接串交接方式**：这个 VM 的每次命令调用都是独立的新 shell，不能像"Teddy 在本机终端里 `export` 一个环境变量、Claude Code 在同一个终端里接着用"那样共享状态，也没有共享终端可以给 Teddy 输入。可行方案：请 Teddy 直接在 `C:\Users\teddy\Downloads\` 目录下新建一个纯文本文件（例如 `.nianlife_source_url.txt`），只在里面粘贴连接串，不要发到对话里；Claude 会用 `$(cat 文件路径)` 的方式在命令内部读取这个文件的内容用于连接，连接串本身不会出现在 Claude 写的命令文字里、不会被回显、不会进入本文档或聊天记录。执行完成后 Claude 会把这个文件内容覆盖清空（这个环境目前没有删除权限，只能清空内容，不能真正删除文件本身），也请 Teddy 之后自行删除这个文件作为兜底。如果总指挥有更好的交接方式，请直接指示。
2. **第二独立存储位置**：本文档 3.7 节要求加密归档 A/B 位于两个独立位置，不能是同一块磁盘上的另一个文件夹。目前只确认了 `Downloads` 文件夹可写（用于归档 A），**尚未收到第二个独立位置**。按 3.7.4 的既定规则，如果执行时仍未确定第二位置，会先完成备份、两次隔离恢复、对账和归档 A，把任务状态记为"部分完成：等待第二位置"，不会用同一磁盘的另一个目录冒充独立副本。

在以上两点明确之前，**不会连接 Neon、不会导出生产数据库**，第 3 节的实际执行仍未开始。
